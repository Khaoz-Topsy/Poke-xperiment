...
export class GoogleAuthService {
  ...
  init = (loginStarted: () => void, navigateOnSuccess: () => void) => {
  ...
  initButton = (elemId: string, loginStarted: () => void, navigateOnSuccess: () => void) => {
    this.init(loginStarted, navigateOnSuccess);
    window.google.accounts.id.renderButton(document.getElementById(elemId), {
      theme: 'filled_black',
      size: 'large',
      shape: 'pill',
      text: 'continue_with',
  ...
  handleResponse =
    (loginStarted: () => void, navigateOnSuccess: () => void) =>
