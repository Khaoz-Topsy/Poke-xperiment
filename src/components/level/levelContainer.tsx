      ...
      return (
        <Flex w="100%" h="100vh" justifyContent="center" flexDirection="column" position="relative">
          ...
          <Center class="controls">
            ...
            <Button onClick={() => props.setUiScale(props.uiScale + 10)} mr="2px">
              +
            </Button>
            <Button onClick={() => props.setUiScale(props.uiScale - 10)}>-</Button>
          </Center>
          <Show when={props.leftControls != null}>{props.leftControls}</Show>
          <Center class="level-children-wrapper" flexGrow={1} position="relative">
            {children}
          </Center>
          <Show when={props.rightControls != null}>{props.rightControls}</Show>
          <Show when={props.bottomControls != null} fallback={<PageHeader text=""></PageHeader>}>
  ...
  return (
    <>
      {applyWrapper(
        ...
        >
          <LevelBackground defaultBackgroundTile={props.defaultBackgroundTile} />
          {props.children}
        </div>,
      )}
    </>
