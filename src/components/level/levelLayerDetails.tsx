import { Box, Button, Divider, Flex, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Spacer, Text, VStack, createDisclosure } from '@hope-ui/solid';
...
export const LevelLayerDetails: Component<ILevelLayerControlProps> = (props: ILevelLayerControlProps) => {
    ...
    createEffect(() => {
        ...
        const currentItemObj = props.levelData?.layers?.[props.selectedLayerIndex]?.items?.find(i => i.id == currentItemId);
            ...
            <Box class="layer-controls-container left">
                <Flex justifyContent="center" flexDirection="column" class="layer-details" gap="5px">
                    <Show when={props.selectedSpriteItemToPaste != null}>
                        <Text textAlign="center" size="xl">Sprite data:</Text>
                        <Box class={(props.selectedSpriteItemToPaste?.width ?? 0) < 4 ? 'zoom-item-2' : ''} mx="auto">
                        ...
                        <Text textAlign="center"><span>width: </span><b>{(props.selectedSpriteItemToPaste?.width ?? 0) * unitInPx}px</b></Text>
                        <Text textAlign="center"><span>height: </span><b>{(props.selectedSpriteItemToPaste?.height ?? 0) * unitInPx}px</b></Text>
                        <Button colorScheme="neutral" mt="0.5em" onClick={props.unselectSpriteItem}>Un-select</Button>
                        <Divider my="1em" />
                    </Show>

                    <Show when={currentItemOnLayer() != null}>
                        <Text textAlign="center" size="xl">Selected Sprite:</Text>
                        ...
                        <Text textAlign="center"><span>x: </span><b>{(currentItemOnLayer()?.x ?? 0)}</b></Text>
                        <Text textAlign="center"><span>y: </span><b>{(currentItemOnLayer()?.y ?? 0)}</b></Text>
                        <Text textAlign="center"><span>width: </span><b>{(currentItemOnLayer()?.width ?? 0) * unitInPx}px</b></Text>
                        <Text textAlign="center"><span>height: </span><b>{(currentItemOnLayer()?.height ?? 0) * unitInPx}px</b></Text>
                        <Button colorScheme="neutral" mt="0.5em" onClick={props.unselectSpriteItem}>Un-select</Button>
                        <Divider my="1em" />
                    </Show>

                    <Show when={currentLayer() != null}>
                        <Text textAlign="center" size="xl">Layer data:</Text>
                        <Text textAlign="center"><span>id: </span>{currentLayer()?.id}</Text>
                        <Text textAlign="center"><span>name: </span>{currentLayer()?.name}</Text>
                        <Text textAlign="center"><span>items: </span>{currentLayer()?.items?.length}</Text>
                        <Text textAlign="center"><span>zIndex: </span>{currentLayer()?.zindex ?? 0}</Text>

                    </Show>

                    <Spacer />
                    <Divider my="1em" />
                    <Text textAlign="center" size="xl">Controls:</Text>
                    <Show when={canSelectSprite()}>
                        <Text><span>Left click</span>: Select sprite on current layer</Text>
                    </Show>
                    <Show when={canDeSelectSprite()}>
                        <Text><span>Left click</span>: Select or deselect sprite on current layer</Text>
                    </Show>
                    <Show when={canPlaceSelectSprite()}>
                        <Text><span>Left click</span>: Place selected sprite on current layer</Text>
                    </Show>
                    ...
                    <Show when={canNudgeSelectSprite()}>
                        <Text><span>Arrow keys</span>: Nudge select sprite ini desired direction on current layer</Text>
                    </Show>

                    <Show when={canDeleteItem()}>
                        <Text><span>Right click</span>: Delete highlighted item on current layer</Text>
                    </Show>
                    <Show when={canDeleteWalkableSection()}>
                        <Text><span>Right click</span>: Delete highlighted walkable section</Text>
                    </Show>
