...
export const LevelControlPasteGrid: Component<IProps> = (props: IProps) => {
    ...
    createEffect(() => {
        const basCssSelector = '.level-container .level-layer.editor-paste.colspan-<colnum> .sprite-tile:hover <siblingCss> + .sprite-tile:after { opacity: 0.5; }';
        ...
        for (let rowIndex = 0; rowIndex < rowLoopCount; rowIndex++) {
            ...
            for (let colIndex = 0; colIndex < colSpanCount; colIndex++) {
                ...
                const siblingCss = Array.from(Array(numSibling + colIndex).keys()).map(d => '+ .sprite-tile').join(' ');
